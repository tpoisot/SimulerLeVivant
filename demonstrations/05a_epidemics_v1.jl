...
function move!(walker::Walker, moves, grid_size)
	move = rand(moves)
	walker.x += move[1]
	walker.y += move[2]
	walker.x = min(grid_size[1], max(1, walker.x))
	walker.y = min(grid_size[1], max(1, walker.y))
	return walker
end

# ╔═╡ c317d977-9370-4df3-a94a-4777afc13be4
function simulation!(population, moves, grid_size)
	population[1].infected = true
	population_size = [length(population)]
	infections = Tuple{Int64, Int64, Int64}[]
	infected = count(x -> x.infected, population)
	infectious = [infected]
	cursor = 0
	while infected > 0
		...
		for individual in population
			if individual.infected
				...
				for other in population
					if other.x == individual.x
						if other.y == individual.y
							other.infected = true
							push!(infections, (individual.x, individual.y, cursor))
						end
					end
				end
			end
		end
		dead = findall(x -> x.clock == 0, population)
		deleteat!(population, dead)
		push!(population_size, length(population))
		infected = count(x -> x.infected, population)
		push!(infectious, infected)
		cursor += 1
	end
	return population_size, infectious, unique(infections)
end

# ╔═╡ da26ab84-978c-47d9-adc2-71783e7ce763
function simstats(N, A, I)
	return (
		popsize = first(N),
		duration = length(N),
		survival = last(N)/first(N),
		maxinf = findmax(A)[1],
		maxwhen = findmax(A)[2],
		peak = findmax(A)[1] / N[findmax(A)[2]]
	)
end

# ╔═╡ 097bc2d9-70cc-4c6d-a73f-a02e7975f2cd
md"""
Number of individuals: $(@bind N0 NumberField(1:prod(grid_size), default=0.24*prod(grid_size)))
"""

# ╔═╡ 6a14ec3f-e6ab-4016-9e4f-8b5195e18133
md"""
with $(N0) individuals, the population density is $(N0/prod(grid_size))
"""

# ╔═╡ bf080841-393f-42a1-9de0-46ea6bd339eb
population = [Walker() for _ in 1:N0]

# ╔═╡ c319238b-5acc-4aad-821d-a6438eddefa5
N, A, I = simulation!(population, moves, grid_size)

# ╔═╡ 0569288f-6df7-4224-a9a2-d50c1d2a72fc
md"""
the epidemic lasted for $(length(N)) clicks, and the survival rate is $(round((last(N)/N0)*100; digits=2))%
"""
...
# ╔═╡ e9c6fbd2-13ac-4428-9b0c-217ec68a068a
plot_scale = [identity => "linéaire", log10 => "log 10", log2 => "log 2", sqrt => "Racine carrée"]

# ╔═╡ 1b9ea1d2-bee7-47b2-b136-7a6aa981c22c
plot_data = [
	("Durée", [o.duration for o in outputs]) => "Durée",
	("Max. prevalence", [o.peak for o in outputs]) => "Prévalence max.",
	("Temps max.", [o.maxwhen for o in outputs]) => "Pic épidémique",
	("Cas simultanés", [o.maxinf for o in outputs]) => "Cas simultanés",
	("Dens. pop.", [o.popsize for o in outputs]./prod(grid_size)) => "Densité de pop.",
	("Taux de survie", [o.survival for o in outputs]) => "Taux de survie",
]

# ╔═╡ 2e9c0033-d2dd-4a82-94bc-666066f32859
md"""
**Axe x**: $(@bind xvar Select(plot_data)) $(@bind xscale Select(plot_scale))

**Axe y**: $(@bind yvar Select(plot_data)) $(@bind yscale Select(plot_scale))

**Couleur**: $(@bind cvar Select(plot_data))
"""
...
begin
	...
	vs = predict(model, us)
	f2 = Figure()
	ax2 = Axis(f2[1,1]; xlabel=xvar[1], ylabel=yvar[1], xscale=xscale, yscale=yscale)
	cl = scatter!(ax2, x, y, color=s, colormap=:navia, colorrange=extrema(s), strokecolor=:darkgrey, strokewidth=0.5)
	lines!(us, vs, color=:lightgrey, linewidth=2, linestyle=:dash)
	Colorbar(f2[1,2], cl; label=cvar[1])
	current_figure()
end
