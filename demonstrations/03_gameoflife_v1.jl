...
# ╔═╡ cb566a78-b2ee-11ee-3c57-b77ca554acd4
using PlutoUI

# ╔═╡ b80d7ded-6581-403a-aa60-146803aa8dc3
using CairoMakie

# ╔═╡ b0c529db-8271-475c-950d-996056511503
md"""
## Définition de la lattice

Lignes: $(@bind rows NumberField(30:200, default=50))
Colonnes: $(@bind cols NumberField(30:200, default=50))

"""

# ╔═╡ 036c3ca9-b783-4f86-b882-6674f732b9e8
lattice = zeros(Bool, (rows, cols))

# ╔═╡ 059966e9-eb32-4394-9689-127542dae1a6
future = copy(lattice)

# ╔═╡ 0c820538-6e68-4be2-9313-2ca9572cfc12
neighbors = zeros(UInt8, size(lattice))

# ╔═╡ 4d158746-0638-4e2f-b1c3-e180c54af989
function initial_state!(lattice, motif, position)
	lattice[position .+ CartesianIndices(motif)] .= motif
	return lattice
end

# ╔═╡ cdce0f5b-1679-4408-9262-37bbca0a3529
stencil = [
	CartesianIndex(-1,-1),
	CartesianIndex(-1,0),
	CartesianIndex(-1,1),
...
# ╔═╡ ac12204c-0f6d-4df3-992d-6750e2267b2c
"""
	number_of_active_neighbors!(neighbors, lattice)

Cette fonction va compter le nombre de voisins de chaque cellule de `lattice`, et *modifier* `neighbors`.
"""
function number_of_active_neighbors!(neighbors, lattice)
	for cell in CartesianIndices(lattice)
		neighborhood = cell .+ stencil
		neighborhood = filter(n -> n in CartesianIndices(lattice), neighborhood)
		neighbors[cell] = sum(lattice[neighborhood])
	end
	return neighbors
end

# ╔═╡ 66abe8b1-b84b-4aad-abfd-465ac1987a4b
function update_state!(future, current, neighbors)
	fill!(future, false)
	number_of_active_neighbors!(neighbors, current)
	for cell in eachindex(current)
		if current[cell]
	...
	for cell in eachindex(future)
		current[cell] = future[cell]
	end
	return current
end

# ╔═╡ 3973176a-6832-4013-ad4b-9ea33e2612d5
spaceship = [false false true; true false true; false true true]

# ╔═╡ db894bf0-4c8a-4b28-88ef-ef2747c66cec
lwss = [false true true true true; true false false false true; false false false false true; true false false true false]

# ╔═╡ 53c3869e-8f28-4463-998b-be61d464906f
@bind initial Select([spaceship => "spaceship", lwss => "LWSS"])

# ╔═╡ 196f6448-c143-412e-b401-371c69434b61
begin
	fill!(lattice, false)
...
manifest_format = "2.0"
project_hash = "a3b1f37210eca24933b72bff8481f4f358552fa1"
