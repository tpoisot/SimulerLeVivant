...
# ╔═╡ 5766c847-f851-4d70-b927-d47504ec5dd4
using PlutoUI

# ╔═╡ cc57df21-eba8-46f3-8fb5-65c7c2d79b1c
using CairoMakie

# ╔═╡ 5dbbf578-b267-11ee-0ff3-918df7eb3ce2
function ΔN(n; r=0.2, K=1.2)
	return r * n * (1 - n/K)
end

# ╔═╡ b294783d-30cf-4b0a-b77e-a1e815ffe4aa
md"""
Initial population: $(@bind x0 Scrubbable(0.1:0.1:2.0; default=0.1))

Growth rate: $(@bind r Scrubbable(0.1:0.1:4.0; default=1.0))

Carrying capacity: $(@bind K Scrubbable(0.8:0.1:2.0; default=1.0))

Timesteps: $(@bind t Scrubbable(10:100; default=50))
"""

# ╔═╡ d0f418e8-eb68-4b21-95be-997067c40fba
@bind run PlutoUI.Button("Simulation")

# ╔═╡ 81d5e783-2f70-43b8-82f2-b111f2497327
let
	run
	population = fill(x0, t+1)
	for i in 1:t
		population[i+1] += ΔN(population[i]; r=r, K=K)
	end
	scatterlines(population; color=:black)
end
